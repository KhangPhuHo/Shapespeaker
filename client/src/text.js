...
function displayProfile() {
    ...
    if (userInfo) {
        ...
        profileContainer.innerHTML = `<p>Hãy tạo tài khoảng</p>`;
    }
}

function updateUserCredentials(userInfo) {
    let storedUser = JSON.parse(localStorage.getItem('user'));

    if (storedUser) {
        storedUser.username = userInfo.username;
        storedUser.avatar = userInfo.avatar;
...
function updateUserInfoAfterLogin() {
    ...
    if (userInfo) {
        const userProfile = {
            ...
            avatar: userInfo.avatar || "" // Đảm bảo avatar không bị null
        };
